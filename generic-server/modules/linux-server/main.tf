...
## ..:: Creating Public IP, vnic ::..
## ----------------------------------------------------------------------------
resource "azurerm_public_ip" "linuxvm_public_ip" {

  count = var.how_many_servers

  name                = "${var.prefix_name}-${var.linuxvm_hostname[count.index]}-public-ip"
...
resource "azurerm_network_interface" "linuxvm_subnet_vnic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.linuxvm_public_ip[count.index].id
  }
